...
--
	...
	function xcode.getfiletype(node)
		local types = {
			[".c"]         = "sourcecode.c.objc", --akovalovs: changed so that c compiles as objective c (instead of "sourcecode.c.c")
			[".cc"]        = "sourcecode.cpp.cpp",
			[".cpp"]       = "sourcecode.cpp.objcpp", --akovalovs: changed so that cpp compiles as object cpp
			[".css"]       = "text.css",
		...
		tree.traverse(tr, {
			onleaf = function(node)
				...
				-- something else
					...
					-- akovalovs: replaced lastKnownFileType with explicitFileType - meaning that user asked xcode to build the file that way
					-- i dont know if lastKnownFileType even matters..
					...
					print(string.format('%s /* %s */ = {isa = PBXFileReference; explicitFileType = %s; name = "%s"; path = "%s"; sourceTree = "<%s>"; };',
						node.id, node.name, xcode.getfiletype(node), node.name, pth, src))
				end
			end
		})
