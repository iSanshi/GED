...
function compactChildren( oOb, oLev, oBsID, oCol, oPM, oT, oML ) {
	...
	for( var x = 0, y = oOb.childNodes; x < y.length; x++ ) { if( y[x].tagName ) {
        ...
        text = text + 1
        //if (text % 2)
        {if( theNextUL/* && theNextUL.id.indexOf('-pe-no-collapse') == -1*/) {
            ...
            y[x].childNodes[0].id = theNextUL.id;
			theNextUL.MWJuniqueID = oIcount++;
			compactChildren( theNextUL, oLev + 1, oBsID, oCol, oPM, oT, oML );
            }
} } } }
function showBlockAndChildren(oThisOb, oShow, oT)
{
    oThisOb.style.display = oShow ? 'block' : 'none';
    if (oShow)
    {
        for( var x = 0, y = oThisOb.childNodes; x < y.length; x++ ) { if( y[x].tagName )
        {
            var theNextUL = y[x].getElementsByTagName( oT )[0];
            if( theNextUL && theNextUL.id.indexOf('-pe-no-collapse') != -1)
            {
                showBlockAndChildren(theNextUL, true, oT);
            }
        }}
    }
}
function clickSmack( oThisOb, oLevel, oBsID, oCol, oT ) {
    ...
    {
        showBlockAndChildren(oThisOb, oThisOb.style.display != 'block', oT);
    }
}
