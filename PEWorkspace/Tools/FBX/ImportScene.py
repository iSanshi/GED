...
def DisplayNodeContent(lScene, pNode, skelRoots, skelDirectMaps, meshToBindMatrixMap, parentName, peParserCtx):
    ...
    for k in ['name', 'fullName', 'translation', 'rotation', 'rotationMatrix', 'scale',
        'preRotation', 'postRotation', 'postRotationMatrix', 'postRotationMatrixFixup',
        'postRotMatrixRotOrder',
        ...
        'transformInheritance', 'rotOrder', 'rotOrderLimitsOnly', 'xformScaleOffsetFixup',
        'xform', 'xformTest', 'xformNoOrder',
        'finalLocalMatrix', 'finalWorldMatrix', 'postRotationMatrixFixupXrotationPivotMatrix',
        'postRotationMatrixFixupXscalingOffsetMatrix']:
...
def DisplayGeometricTransform(pNode, peParserCtx, nodeDict):
    ...
    postRot = pNode.GetPostRotation(FbxNode.eSourcePivot) #rotate axis
    ...
    postRotMatrixRotOrder = ConstructRotationMatrix(postRot[0], postRot[1], postRot[2], nodeDict['rotOrder'])
    nodeDict['postRotMatrixRotOrder'] = postRotMatrixRotOrder
    postRotationMatrixFixup = postRotMatrix.Inverse() #for some reason, fbx stores maya node's "Rotate Axis" as Inverse of Matrix("Rotate Axis")
    nodeDict['postRotationMatrixFixup'] = postRotationMatrixFixup
    ...
    #this one works for rpivot at 0,0,0
    #fixedUpScalingOffsetMatrix = rotationMatrix.Inverse() * rotationMatrixNoOrder * scalingOffsetMatrix
    #this one works with rpivot anywhere, but with no rotate axis
    #fixedUpScalingOffsetMatrix = rotationPivotMatrix * rotationMatrix.Inverse() * rotationPivotMatrix.Inverse() * rotationPivotMatrix * rotationMatrixNoOrder * rotationPivotMatrix.Inverse() * scalingOffsetMatrix
    
    combinedRotationMatrix = preRotMatrix * rotationMatrix * postRotationMatrixFixup
    combinedRotationMatrixNoOrder = preRotMatrix * rotationMatrixNoOrder * postRotationMatrixFixup
    
    #fixedUpScalingOffsetMatrix = rotationPivotMatrix * combinedRotationMatrix.Inverse() * rotationPivotMatrix.Inverse() * rotationPivotMatrix * combinedRotationMatrixNoOrder * rotationPivotMatrix.Inverse() * scalingOffsetMatrix
    fixedUpScalingOffsetMatrix = rotationPivotMatrix * rotationMatrix.Inverse() * rotationPivotMatrix.Inverse() * rotationPivotMatrix * rotationMatrixNoOrder * rotationPivotMatrix.Inverse() * scalingOffsetMatrix 
    ...
    #scalingPivotPartMatrix = postRotationMatrixFixup * scalingPivotPartMatrix
    #scalingPivotPartMatrix.SetRow(0, FbxVector4(1.0, 0, 0, 0));
    #scalingPivotPartMatrix.SetRow(1, FbxVector4(0, 1.0, 0, 0));
    #scalingPivotPartMatrix.SetRow(2, FbxVector4(0, 0, 1.0, 0));
    
    xform = translationMatrix \
        * rotationOffsetMatrix * rotationPivotMatrix * preRotMatrix * rotationMatrix * postRotationMatrixFixup * rotationPivotMatrix.Inverse() \
        * scalingOffsetMatrix * scalingPivotPartMatrix * scaleMatrix * scalingPivotPartMatrix.Inverse()
    
    xformScaleOffsetFixup = translationMatrix \
        * rotationOffsetMatrix * rotationPivotMatrix * preRotMatrix * rotationMatrix * postRotationMatrixFixup  * rotationPivotMatrix.Inverse() \
        * fixedUpScalingOffsetMatrix * scalingPivotPartMatrix * scaleMatrix * scalingPivotPartMatrix.Inverse()
    
    xformTest = translationMatrix \
        * rotationOffsetMatrix * rotationPivotMatrix * preRotMatrix * rotationMatrix * postRotationMatrixFixup  * rotationPivotMatrix.Inverse() \
        * fixedUpScalingOffsetMatrix * scalingPivotPartMatrix * scaleMatrix * scalingPivotPartMatrix.Inverse()
    
    combinedRotationMatrix = postRotationMatrixFixup * rotationMatrix * preRotMatrix
    combinedRotationMatrixNoOrder = postRotationMatrixFixup * rotationMatrixNoOrder * preRotMatrix
    
    otherVersion5 = translationMatrix \
        * rotationOffsetMatrix * rotationPivotMatrix * combinedRotationMatrixNoOrder * rotationPivotMatrix.Inverse() \
        * scalingOffsetMatrix * scalingPivotPartMatrix * scaleMatrix * scalingPivotPartMatrix.Inverse()
    

    nodeDict['xformScaleOffsetFixup'] = xformScaleOffsetFixup
    
    nodeDict['finalLocalMatrix'] = xformScaleOffsetFixup
    
    nodeDict['xform'] = xform
    
    nodeDict['xformTest'] = xformTest
    nodeDict['xformNoOrder'] = otherVersion5

    
    nodeDict['postRotationMatrixFixupXrotationPivotMatrix'] = postRotationMatrixFixup * rotationPivotMatrix
    
    nodeDict['postRotationMatrixFixupXscalingOffsetMatrix'] = postRotationMatrixFixup * scalingOffsetMatrix
    
    nodeDict['finalWorldMatrix'] = nodeDict['finalLocalMatrix']
    if nodeDict['parent']:
        nodeDict['finalWorldMatrix'] = nodeDict['parent']['finalWorldMatrix'] * nodeDict['finalLocalMatrix']
